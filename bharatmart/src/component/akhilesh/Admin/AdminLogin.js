...
//   import Footer from '../Kaushik/Footer';
  ...
  const login=async(data)=>{
    ...
    try {
      const res = await fetch("http://localhost:5000/admin/login1", {
        method: "POST",
        headers: {
  ...
  export default function Login() {
    ...
    login(data).then(res=>{
      console.log("admin----",res.userDetails)
      const {_id}=res.userDetails
      // const {userdetails}=res;
      // const {_id}=userdetails;
      // console.log("admin--->>>>.-",_id)
      localStorage.setItem("token",JSON.stringify(res.token))
      localStorage.setItem("Admin",JSON.stringify(_id));
      console.log(res,"login  is ")
      toast({
        title:`${res.message}`,
        description: "Thanku",
        status: 'success',
          ...
          <Box
           ...
           <Stack spacing={5} pt={2}>
              ...
              <Text align={'center'} color={"black"} marginTop={"0px"} fontWeight="bold">
               Not have a account? <Link to="/admin">Signup</Link>
              </Text>
            </Stack>
              </form>
            </Stack>
